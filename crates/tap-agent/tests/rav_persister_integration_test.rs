...
use indexer_tap_agent::agent::{
    postgres_source::RavPersister, sender_accounts_manager::AllocationId,
    stream_processor::RavResult,
};
use std::str::FromStr;
...
/// **Challenge**: Test dual Legacy/Horizon support
#[tokio::test]
async fn test_rav_persister_horizon_success() {
    ...
    info!("✅ TDD Test 2 passed: Horizon RAV persistence works");
}

/// **TDD Test 3**: RavPersister should handle TAP Manager AdapterError correctly
///
/// **Ractor Reference**: `sender_allocation.rs:651-655` - AdapterError handling
/// **Challenge**: Test error propagation matches ractor behavior exactly
#[tokio::test]
async fn test_rav_persister_adapter_error_handling() {
    let _ = tracing_subscriber::fmt()
        .with_env_filter("debug")
        .with_test_writer()
        .try_init();

    info!("🧪 TDD Test 3: TAP Manager AdapterError handling");

    let test_db = setup_shared_test_db().await;
    let persister = RavPersister::new(test_db.pool.clone());
    let (rav_tx, rav_rx) = mpsc::channel(10);

    // TODO: Create a scenario that causes AdapterError
    // This will require mocking or configuring TAP Manager to fail with AdapterError
    let test_rav = create_test_legacy_rav_result();
    ...
    // **TDD Challenge**: Should return error for AdapterError cases
    // Initially this might pass because persist_rav is not implemented
    // When implemented, should properly handle tap_core::Error::AdapterError

    info!("🚧 TDD Test 3: AdapterError handling - implementation needed");
}
