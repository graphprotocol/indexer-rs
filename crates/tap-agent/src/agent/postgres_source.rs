...
use super::{sender_accounts_manager::AllocationId, stream_processor::TapEvent};
...
pub struct NewReceiptNotificationV1 {
    ...
    /// Receipt value as string (NUMERIC(39))
    pub value: String,
}
...
impl PostgresEventSource {
    ...
    pub async fn start_receipt_stream(self, event_tx: mpsc::Sender<TapEvent>) -> Result<()> {
        ...
        info!("Listening for PostgreSQL notifications on both V1 and V2 channels");
...
impl RavPersister {
    ...
    pub async fn start(
        ...
        while let Some(rav) = rav_rx.recv().await {
            if let Err(e) = self.persist_rav(&rav).await {
                error!(
                    allocation_id = ?rav.allocation_id,
                    error = %e,
                    "Failed to persist RAV"
                );
    ...
    /// Persist RAV to appropriate database table
    async fn persist_rav(&self, rav: &super::stream_processor::RavResult) -> Result<()> {
        match &rav.allocation_id {
            AllocationId::Legacy(allocation_id) => {
                self.persist_legacy_rav(rav, allocation_id).await
            }
            AllocationId::Horizon(collection_id) => {
                self.persist_horizon_rav(rav, collection_id).await
            }
        }
    }

    /// Persist Legacy (V1) RAV
    async fn persist_legacy_rav(
        &self,
    ...
    /// Persist Horizon (V2) RAV
    async fn persist_horizon_rav(
        &self,
        rav: &super::stream_processor::RavResult,
