        ...
        loop {
            tokio::select! {
                ...
                notification = listener_v1.recv() => {
                    match notification {
                        Ok(notification) => {
                            debug!(channel = notification.channel(), payload = notification.payload(), "🔔 Received V1 notification from PostgreSQL");
                ...
                notification = listener_v2.recv() => {
                    match notification {
                        Ok(notification) => {
                            debug!(channel = notification.channel(), payload = notification.payload(), "🔔 Received V2 notification from PostgreSQL");
