...
# Check if tap-agent is running and processing receipts
# This is a basic check - you can expand it based on your specific tap-agent API
TAP_AGENT_STATUS=$(curl -s -f http://tap-agent:7602/status 2>/dev/null || echo "Connection failed")

if echo "$TAP_AGENT_STATUS" | grep -q "Connection failed"; then
    echo "⚠️ Could not connect to tap-agent status endpoint (this might be expected if no status endpoint exists)"
else
    echo "✅ TAP agent status: $TAP_AGENT_STATUS"
fi
